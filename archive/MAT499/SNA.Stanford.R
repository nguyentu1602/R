...
###
# 0.BASICS AND LOADING GRAPH DATA INTO R OBJECTS
###

# To detach a package: 
detach(package:RCytoscape)

# To read data directly from URL:
...
# To assign column names to data
...
# To check $ego and $alter are the same across datasets and then merge them:
...
# To rename the last 2 columns:
names(krack_full_data_frame)[4:5] <- cbind('friendship_tie','reports_to_tie')

# To reduce to non-zero edges so that the edge list only contains actual ties of some type.
krack_full_nonzero_edges <- subset(krack_full_data_frame, 
                                   (advice_tie > 0 | friendship_tie > 0 | reports_to_tie > 0))

# To import this dataframe into a 'graph' object in igraph: using graph.data.frame()
...
# *any type* of tie. The tie types themselves are listed as edge attributes.

# To get a vector of edges for a specific type of tie, use get.edge.attribute()
...
# To symmetrize the network, making all asymmetric ties symmetric, use the as.undirected():
krack_full_symmetrized <- as.undirected(krack_full, mode='collapse')
summary(krack_full_symmetrized)

###
# 1. ADDING VERTEX ATTRIBUTES TO A GRAPH OBJECTS
###

# One way to add the attributes to your graph object is to iterate through each attribute and each 
# vertex. This means that we will add one attribute at a time to each vertex in the network.
# V(krack_full) returns a list of the IDs of each vertex in the graph. 
# names(attributes) returns a list of the column names in the attributes table. 
# The double-for loop tells R to repeat the code between the brackets once for each attribute and 
...
# clicking and selecting "Properties."  The path information is listed "location".
